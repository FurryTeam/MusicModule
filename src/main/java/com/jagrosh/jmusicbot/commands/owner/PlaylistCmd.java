...
public class PlaylistCmd extends OwnerCommand 
{
    ...
    {
        StringBuilder builder = new StringBuilder(event.getClient().getWarning()+" Управление плейлистами:\n");
    ...
    public class AppendlistCmd extends OwnerCommand 
    {
        public AppendlistCmd()
        {
            ...
            this.help = "добавляет трек в существующий плейлист";
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(parts.length<2)
            {
                event.reply(event.getClient().getError()+" Укажите название плейлиста и ссылку для добавления!");
                return;
            }
            ...
            else
            {
                ...
                try
                {
                    ...
                    event.reply(event.getClient().getSuccess()+" Успешно добавлено "+urls.length+" треков в плейлист `"+pname+"`!");
                }
                catch(IOException e)
                {
                    event.reply(event.getClient().getError()+" Не могу добавить в плейлист: "+e.getLocalizedMessage());
                }
            }
        }
    }
    ...
    public class ListCmd extends OwnerCommand 
    {
        public ListCmd()
        {
            ...
            this.help = "список доступных плейлистов";
            this.guildOnly = true;
        }
        ...
        protected void execute(CommandEvent event) 
        {
            ...
            if(!bot.getPlaylistLoader().folderExists())
            {
                event.reply(event.getClient().getWarning()+" База плейлистов не существует и не может быть создана!");
                return;
            }
            List<String> list = bot.getPlaylistLoader().getPlaylistNames();
            if(list==null)
                event.reply(event.getClient().getError()+" Ошибка загрузки доступных плейлистов!");
            else if(list.isEmpty())
                event.reply(event.getClient().getWarning()+" В базе данных нет ни одного плейлиста!");
            else
            {
